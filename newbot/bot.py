...
async def clear_games_command(message: Message):
    ...
    Якщо ігор немає — повідомляє про це.
    Якщо є — показує підтвердження очищення.
    """
    user_id = str(message.from_user.id)
    logger.info(f"User {user_id} requested clear games")

    games = read_user_data(user_id)

    if not games:
        await message.answer("❌ У вас немає ігор для видалення.")
        return
